/*********************************************************************** 
   W32pthread.c - FORTRAN/C interface for Windows NT/95 Xplot11
...
C     Version 4.60 05/07/2025 
*
*  Status: This code is working but there are still some
*          rough edges some of which result from issuess with the Win32
...
*
*          Problems with compiling for 64-bits were found, related to the
*          use of Win32 threading that affect mouse and key inputs.  The
*          solution was to switch to Pthreads for thread creation. If 32-bit
*          compilers are used the older Win32 threads work fine, for 64-bit
*          executables the pthreads version is needed.
*
*          Harold Youngren 05/25
***********************************************************************/
...
gwxAddEvent(int type, int x, int y, int state)
{
    ...
    /* wait for blocked thread */
    //    printf("gwxAddEvent blockflag %d\n",blockflag);
    while(blockflag) {
      Sleep(1);
    }
    /* take control of shared data block flag */
    blockflag = 1;

    /* make new event */
    event = (Event *) malloc(sizeof(Event));
    if (event == NULL) {
      //      printf("Allocation of new event failed\n");
      blockflag = 0;
      return;
    }
    ...
    gwxLast = *event;
    //    printf("New event type %d x %d y %d\n",type,x,y);
    blockflag = 0;
}
...
gwxRemEvent(int *type, int *x, int *y, int *state)
{
    ...
    /* wait for blocked thread */
    //    printf("gwxRemEvent blockflag %d\n",blockflag);
    while(blockflag) {
      Sleep(1);
    }
    /* take control of shared data block flag */
    ...
    *state = event->state;
    free(event);
    //    printf("Event removed type %d x %d y %d\n",type,x,y);
    blockflag = 0;
}
...
gwxWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_ACTIVATE:
      //        printf("WM_ACTIVATE blockflag %d\n",blockflag);
        break;
    ...
    case WM_MBUTTONDOWN:
      //	printf("MbuttonDown %d  %d\n",LOWORD(lParam), HIWORD(lParam)); 
        gwxAddEvent(XButtonPress, LOWORD(lParam), HIWORD(lParam), 2);
        return 0;

    case WM_MBUTTONUP:
      //	printf("MbuttonUp %d  %d\n",LOWORD(lParam), HIWORD(lParam)); 
        gwxAddEvent(XButtonRelease, LOWORD(lParam), HIWORD(lParam), 2);
        return 0;

    case WM_RBUTTONDOWN:
      //	printf("RbuttonDown %d  %d\n",LOWORD(lParam), HIWORD(lParam)); 
    ...
    case WM_CHAR:
        ...
        if ((gwxLast.type !=  XKeyPress) || (gwxLast.state < 352) || 
...
GWXNUMEVENTS( void )
{
    ...
    //    if(blockflag) printf("gwxNumEvents blockflag set\n");
    while(blockflag) {
       Sleep(1);
 }
    stat = 1;
    if (gwxEvents == NULL) stat = 0;
    //    printf("gwxNumEvents status %d\n");
    //    Sleep(2);
    Sleep(1);
    //    blockflag = 0;
    return stat;
...
{
  //    if(blockflag) printf("gwxEvent blockflag set\n");
...
GWXGETEVENT(int *type, int *x, int *y, int *state)
{
    ...
    /* wait for blocked thread */
    //    printf("gwxGetEvent blockflag %d\n",blockflag);
    while(blockflag) {
      Sleep(1);
    }
    /* take control of shared data block flag */
...
GWXCLEAR( void )
{
    ...
    /* wait for blocked thread */
    //    if(blockflag) printf("gwxClear blockflag set\n");
    while(blockflag) {
      Sleep(1);
    }
    /* take control of shared data block flag */
...
*/
void W32CALL
GWXPOLY(int *x_coord, int *y_coord, int *n_coord)
